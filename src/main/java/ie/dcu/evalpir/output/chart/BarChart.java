...
public class BarChart {
		...
		try {
			...
			if (m.getName().contains("Precision@")) {
				ChartUtilities.saveChartAsPNG(new File(
						path + "/Precision@/" + "User_" + user + "_Topic_" + nameTopic + "_" + m.getName() + ".png"),
						chart, widthChart, heightChart, null, false, 9);
			} else if (m.getName().contains("Recall@")) {
				ChartUtilities.saveChartAsPNG(new File(
						path + "/Recall@/" + "User_" + user + "_Topic_" + nameTopic + "_" + m.getName() + ".png"),
						chart, widthChart, heightChart, null, false, 9);

			} else {
				ChartUtilities.saveChartAsPNG(new File(path + "/" + m.getName() + "/" + "User_" + user + "_Topic_"
						+ nameTopic + "_" + m.getName() + ".png"), chart, widthChart, heightChart, null, false, 9);
	...
	private static JFreeChart createChart(final CategoryDataset dataset, final String user, final String topicName,
			final String measure, ArrayList<Query> topic) {
		JFreeChart chart = ChartFactory.createBarChart("User: " + user + " Topic: " + topicName, "Systems", measure,
