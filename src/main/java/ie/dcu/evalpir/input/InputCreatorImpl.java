...
public class InputCreatorImpl implements InputCreator {
	...
	static final String[] TOPIC_ID = { "MOVS-AP", "MOVS-YP", "SPRT-YP", "SPRT-KS", "TRVL-BT", "TRVL-VT", "ART-Y",
			"ART-A", "BKS-A", "BKS-Y", "MSC-Y", "MSC-A" };
	...
	public void generateFilesInput(int nUser, int nTopic) {
		...
		generateCSV(qRels, "src/main/resources/qrels.test." + "nUser." + nUser + ".nTopic." + nTopic + "."
				+ time.toString() + ".csv");
		ArrayList<String> result = generateResult(nUser, nTopic);
		generateCSV(result, "src/main/resources/result.test." + "nUser." + nUser + ".nTopic." + nTopic + "."
				+ time.toString() + ".csv");
			...
			for (int j = 0; j < nTopic; j++) {
				for (int y = 0; y < MAX_QUERY; y++) {
					for (int k = 0; k < MAX_EVAL_DOC; k++) {
						...
						inputFile.add(i + ", " + TOPIC_ID[j] + ", " + queryId + ", " + "clueweb12-0100tw-" + documentId
								+ ", " + relevance + "\n");
						documentId++;
					}
					queryId++;
